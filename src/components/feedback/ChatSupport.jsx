...
import { MessageSquare, X, Send, Cpu, Bot, User, Loader2, Sparkles, ChevronDown, BrainCircuit } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-[200]">
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="absolute bottom-16 right-0 w-[calc(100vw-2rem)] sm:w-[350px] md:w-[400px] max-h-[80vh] bg-card-bg rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-border-main overflow-hidden flex flex-col"
          >
            {/* Header */}
            <div className="bg-surface-dark p-6 text-white flex items-center justify-between relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-accent/20 blur-3xl rounded-full"></div>
              <div className="flex items-center gap-4 relative z-10">
                <div className="w-12 h-12 bg-accent rounded-sm flex items-center justify-center shadow-lg shadow-accent/20">
              ...
              <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-card-bg/10 rounded-sm transition-all relative z-10">
                <X className="h-5 w-5" />
              </button>
            </div>

            {/* Messages */}
            <div ref={scrollRef} className="flex-grow h-96 overflow-y-auto p-6 space-y-6 bg-app-bg/50">
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex gap-3 max-w-[85%] ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    <div className={`w-8 h-8 rounded-sm flex items-center justify-center flex-shrink-0 ${m.role === 'user' ? 'bg-surface-dark text-white' : 'bg-card-bg border border-border-main text-accent shadow-sm'}`}>
                      {m.role === 'user' ? <User className="h-4 w-4" /> : <Sparkles className="h-4 w-4" />}
                    </div>
                    <div className={`p-4 rounded-sm text-[11px] font-medium leading-relaxed shadow-sm ${
                      m.role === 'user' 
                        ? 'bg-accent text-white rounded-tr-none' 
                        : 'bg-card-bg text-text-primary border border-border-main rounded-tl-none'
                    }`}>
                      {m.text}
                    </div>
                  </div>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  <div className="flex gap-3 items-center">
                    <div className="w-8 h-8 rounded-sm bg-card-bg border border-border-main text-accent flex items-center justify-center shadow-sm">
            ...
            {/* Input */}
            <form onSubmit={handleSend} className="p-5 bg-card-bg border-t border-border-main flex gap-3 items-center">
              <div className="flex-grow relative">
                <input 
                  ...
                  className="w-full pl-5 pr-12 py-4 bg-app-bg border border-border-main rounded-sm text-xs font-bold outline-none focus:border-accent focus:bg-card-bg transition-all shadow-inner"
                />
                <button type="button" className="absolute right-4 top-1/2 -translate-y-1/2 text-text-muted hover:text-accent transition-colors">
                  <BrainCircuit className="h-5 w-5" />
                </button>
              </div>
              <button type="submit" className="w-14 h-14 bg-surface-dark text-white rounded-sm flex items-center justify-center hover:bg-accent transition-all shadow-lg shadow-slate-900/10 active:scale-95">
      ...
      <button 
        ...
        className="w-20 h-20 bg-accent text-white rounded-sm shadow-2xl flex items-center justify-center hover:scale-105 transition-all group relative active:scale-95 shadow-accent/30"
      >
        ...
        {!isOpen && (
          <span className="absolute -top-2 -right-2 flex h-6 w-6">
            ...
            <span className="relative inline-flex rounded-full h-6 w-6 bg-status-success border-4 border-card-bg"></span>
          </span>
        )}
      </button>
    </div>
