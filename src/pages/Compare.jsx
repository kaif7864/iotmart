...
import { X, ShoppingCart, Info, Scale, Check, Minus } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          ...
          <button 
            ...
            className="text-red-500 hover:text-red-600 text-xs font-bold uppercase tracking-widest flex items-center gap-2 transition-all p-2 hover:bg-red-50 rounded-lg"
          >
            <X className="h-4 w-4" /> Clear All
          </button>
        </div>

        <div className="overflow-x-auto rounded-2xl border border-border-main bg-white shadow-sm">
          <table className="w-full text-left border-collapse">
                ...
                {comparisonList.map((product) => (
                  <th key={product._id} className="py-8 px-8 border-l border-border-main min-w-[250px] group relative">
                    <button 
                      ...
                      className="absolute top-4 right-4 p-1.5 bg-white border border-border-main rounded-full text-text-muted hover:text-red-500 hover:border-red-200 transition-all shadow-sm opacity-0 group-hover:opacity-100"
                    >
                      <X className="h-3 w-3" />
                    </button>
                    <div className="flex flex-col items-center text-center">
                      <div className="w-24 h-24 mb-6 bg-slate-50 rounded-xl p-3 border border-border-main">
                      ...
                      <Link 
                        ...
                        className="w-full py-2.5 bg-text-primary text-white text-[10px] font-bold rounded-lg uppercase tracking-widest hover:bg-accent transition-all shadow-md"
                      >
                        View Details
                      </Link>
                    </div>
            ...
            <tbody className="divide-y divide-slate-100">
              <tr>
                <td className="py-5 px-8 text-xs font-bold text-text-muted uppercase tracking-wider bg-slate-50/50">Category</td>
              ...
              <tr>
                <td className="py-5 px-8 text-xs font-bold text-text-muted uppercase tracking-wider bg-slate-50/50">User Rating</td>
                {comparisonList.map(p => (
              ...
              <tr>
                <td className="py-5 px-8 text-xs font-bold text-text-muted uppercase tracking-wider bg-slate-50/50">Availability</td>
                {comparisonList.map(p => (
                  <td key={p._id} className="py-5 px-8 border-l border-border-main/50">
                    <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded ${p.inStock ? 'text-emerald-600 bg-emerald-50' : 'text-red-500 bg-red-50'}`}>
              ...
              {allSpecs.map(spec => (
                <tr key={spec}>
                  <td className="py-5 px-8 text-xs font-bold text-text-secondary bg-slate-50/50">{spec}</td>
                  {comparisonList.map(p => (
                    <td key={p._id} className="py-5 px-8 border-l border-border-main/50">
