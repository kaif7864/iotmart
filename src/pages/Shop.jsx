  ...
  return (
    <div className="pt-32 pb-32 min-h-screen bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Page Header */}
        <div className="mb-12 flex flex-col md:flex-row justify-between items-end gap-6 border-b border-border-main pb-8">
          <div>
          ...
          <div className="flex bg-white border border-border-main p-1 rounded-xl shadow-sm">
            <button 
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-accent text-white shadow-md' : 'text-text-muted hover:text-accent'}`}
            >
              <Grid className="h-5 w-5" />
            </button>
            <button 
              onClick={() => setViewMode('list')}
              className={`p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-accent text-white shadow-md' : 'text-text-muted hover:text-accent'}`}
            >
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="w-full lg:w-72 flex-shrink-0 space-y-8">
            ...
            {/* Price & Connectivity */}
            <div className="bg-white p-8 rounded-3xl border border-border-main shadow-sm space-y-10">
              {/* Price Range */}
              <div>
                ...
                <div className="space-y-3">
                  {[4, 3, 2].map(star => (
                    <button 
                      ...
                      className={`flex items-center gap-2 w-full p-2 rounded-lg transition-all ${selectedRating === star ? 'bg-slate-50' : ''}`}
                    >
              ...
              <button 
                ...
                className="w-full py-3 text-[10px] font-black uppercase tracking-[0.2em] text-red-500 hover:bg-red-50 rounded-xl transition-all border border-red-100"
              >
                Clear All Filters
              </button>
            </div>
          ...
          <div className="flex-grow space-y-8">
            {/* Toolbar */}
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 p-4 bg-white rounded-2xl border border-border-main shadow-sm">
              <div className="relative w-full md:w-[400px]">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-text-muted" />
                <input 
                  type="text" 
                  placeholder="Search products..." 
                  className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-border-main rounded-xl text-sm text-text-primary focus:outline-none focus:border-accent transition-all"
                  value={searchTerm}
            ...
            {loading ? (
                    ...
                    {displayedProducts.map(product => (
                      ...
                      >
                        {viewMode === 'grid' ? (
                          ...
                          <div className="bg-white p-6 rounded-2xl border border-border-main flex flex-col md:flex-row items-center gap-8 group hover:shadow-lg transition-all">
                            <div className="w-32 h-32 bg-slate-50 rounded-xl p-4 flex-shrink-0">
                ...
                {visibleCount < filteredProducts.length && (
                  <div className="text-center pt-8">
                    <button 
                      ...
                      className="px-12 py-4 bg-white border-2 border-border-main text-text-primary rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-slate-50 hover:border-accent transition-all shadow-sm"
                    >
                      Load More Products
                    </button>
                  </div>
              ...
              <div className="text-center py-32 bg-white rounded-3xl border border-dashed border-border-main">
