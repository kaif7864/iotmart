...
import { Package, Truck, CheckCircle, Clock, MapPin, Search, ArrowRight, Loader2, Info } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { getAllOrders } from '../services/api'; // In a real app, we'd have a specific getOrderById
...
const TrackOrder = () => {
  ...
  const handleTrack = async (e) => {
    ...
    try {
      ...
      if (foundOrder) {
        ...
        setError('Order ID not found in our neural network. Please check and try again.');
      }
    } catch (err) {
  ...
  const steps = [
    ...
    { label: 'Processing', icon: Package, color: 'text-blue-500' },
  ...
  return (
    <div className="pt-32 pb-24 min-h-screen bg-slate-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Search Bar */}
        <div className="bg-white p-4 rounded-[32px] shadow-xl border border-border-main mb-12">
          <form onSubmit={handleTrack} className="flex flex-col md:flex-row gap-4">
            <div className="flex-grow relative">
              <Search className="absolute left-6 top-1/2 -translate-y-1/2 h-5 w-5 text-text-muted" />
              <input 
                type="text" 
                placeholder="Enter Neural Tracking ID or Order ID..." 
                className="w-full pl-16 pr-8 h-16 bg-slate-50 border border-transparent rounded-[24px] text-sm font-bold focus:bg-white focus:border-accent transition-all outline-none"
                value={orderId}
        ...
        <AnimatePresence mode="wait">
          ...
          {order && (
            ...
            >
              {/* Order Info Card */}
              <div className="bg-white p-10 rounded-[40px] border border-border-main shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div>
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">Shipping Matrix</p>
                  ...
                  <div className="flex flex-col justify-end items-start md:items-end md:text-right">
                    ...
                    <div className="bg-slate-900 p-6 rounded-3xl w-full max-w-xs text-white">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-1">Items in Transit</p>
              ...
              <div className="bg-white p-10 rounded-[40px] border border-border-main shadow-sm">
                <div className="relative flex justify-between">
                  {/* Background Line */}
                  <div className="absolute top-7 left-0 w-full h-1 bg-slate-100 z-0"></div>
                  <div 
                  ...
                  {steps.map((step, index) => {
                    ...
                    return (
                      <div key={index} className="relative z-10 flex flex-col items-center gap-4">
                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 ${isActive ? 'bg-accent text-white shadow-xl shadow-accent/20 scale-110' : 'bg-white border-2 border-slate-100 text-slate-300'}`}>
                          <Icon className="h-6 w-6" />
                        </div>
                        <div className="text-center">
                          <p className={`text-[9px] font-black uppercase tracking-widest ${isActive ? 'text-text-primary' : 'text-slate-300'}`}>
                            {step.label}
                          </p>
                          {isCurrent && (
